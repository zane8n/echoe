...
export function NotificationSheet({ notifications, readIds, onMarkRead, onSelect, onClose }: Props) {
    ...
    return <><div className="fixed inset-0 z-60 acrylic-backdrop animate-fade-in" onClick={onClose} /><aside className="sheet-surface fixed inset-y-0 right-0 z-70 h-dvh w-[min(100%,440px)] overflow-y-auto acrylic-surface px-[clamp(20px,5vw,36px)] py-6 animate-slide-up" role="dialog" aria-modal="true" aria-label="Notifications"><span className="sheet-grabber" aria-hidden="true" />
        <div className="flex items-start justify-between gap-4 border-b border-[var(--color-line)] pb-5"><div><div className="text-xs font-semibold uppercase text-[var(--color-accent-ink)]">Echoes</div><h2 className="m-0 mt-1 text-[var(--text-xl)] font-semibold">Notifications</h2></div><button className="icon-button" onClick={onClose} aria-label="Close notifications"><Icon name="x" /></button></div>
